...
import { wechatPay } from '../config/config.js';
...
export const getPayParams = async (req, res) => {
    ...
    try {
        ...
        // 更新订单支付方式为微信支付
        await db.query(
            "UPDATE `order` SET pay_method = 'wechat' WHERE order_no = ?",
            [order_no]
        );
...
export const handlePayCallback = async (req, res) => {
    ...
    try {
        ...
        if (decryptedData.trade_state === "SUCCESS") {
            ...
            // 转换时间格式
            const payTime = new Date(success_time).toISOString().slice(0, 19).replace('T', ' ');

            // 5. 更新订单状态
            await db.query(
                "UPDATE `order` SET status = 1, pay_time = ?, pay_method = 'wechat' WHERE order_no = ? AND status = 0",
                [payTime, out_trade_no]
            );
